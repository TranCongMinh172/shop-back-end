...
//        Class<ProductPrice> productPriceClass = ProductPrice.class;
        ...
        StringBuilder queryBuilder = new StringBuilder(getQuery("new com.example.shop.dtos.responses.product.ProductUserResponse(p, pp.discount, pp.discountedPrice, pp.expiredDate)"));
...
//        Class<ProductPrice> productPriceClass = ProductPrice.class;
        ...
        StringBuilder queryBuilder = new StringBuilder(getPromotionQuery("new com.example.shop.dtos.responses.product.ProductUserResponse(p, pp.discount, pp.discountedPrice, pp.expiredDate)"));
        appendQueryBuilder(search, queryBuilder, " %s %sp.%s%s %s ?%s");
        sortBy(queryBuilder, " order by p.%s %s", sort);
        TypedQuery<ProductUserResponse> query = entityManager.createQuery(queryBuilder.toString(), ProductUserResponse.class);
        query.setFirstResult((pageNo - 1) * pageSize);
